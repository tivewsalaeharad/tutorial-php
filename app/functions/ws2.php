...
$socket = stream_socket_server("tcp://127.0.0.1:8090", $errno, $errstr);
...
while (true) {
    ...
    sleep(1);
    sendMessage($connects, 'Сейчас: ' . date('d M Y (D) H:i:s'));
...
function sendMessage($connects, $message)
{
    foreach ($connects as $connect) {
        fwrite($connect, encode($message));
    }
}